...
CLIENT_PATHS = [
    "/Users/sreebhargavibalija/Desktop/uflallfiles/UFM3- FederatedAgents/chestx-ray/clients/reduced_client1"
]
...
# =========================
# 4. Real Differential Privacy: Add Gaussian Noise
# =========================

def add_dp_noise(embeddings, epsilon=2.0, delta=1e-5):
    """
    Add Gaussian noise to embeddings to satisfy (ε, δ)-DP.
    """
    sensitivity = 1.0  # Assuming normalized embeddings (standard in NLP)
    sigma = np.sqrt(2 * np.log(1.25 / delta)) * sensitivity / epsilon
    noisy_embeddings = embeddings + np.random.normal(0, sigma, embeddings.shape)
...
for real_patient_id, img_path in tqdm(patient_image_paths, total=len(patient_image_paths), desc="Processing Patients"):
    ...
    # ✅ Add Real DP Noise to embeddings
    noisy_embeddings = add_dp_noise(embeddings, epsilon=2.0, delta=1e-5)

    # ✅ Mean representation after adding noise
    representative_embedding = np.mean(noisy_embeddings, axis=0)
...
save_path = os.path.join("c1_dp_prompt_patient_embeddings_csv", "c1_text_final_all_patients_embeddings.csv")
